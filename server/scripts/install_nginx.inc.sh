...
function install_nginx()
{
    ...
    install_package ${PACKAGE_DIR}/${NGINX_TAR_NAME} ${NGINX_TAR_NAME} \
        --prefix=${NGINX_DIR} \
    ...
    [ $? -eq 0 ] || exit_with_error "${NGINX_TAR_NAME} cannot be started!"
...
then
    install_nginx
    add_install_record ${NGINX_ID_NAME} ${NGINX_TAR_NAME} ${NGINX_DIR}

else
    echo
    echo "${NGINX_TAR_NAME} has already been installed."
    echo "Nothing to do."
    echo
fi
