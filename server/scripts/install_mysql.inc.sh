...
function install_mysql()
{
    ...
    install_package ${PACKAGE_DIR}/${MYSQL_TAR_NAME} ${MYSQL_TAR_NAME} \
        --prefix=${MYSQL_DIR} \
        --enable-assembler \
        --with-charset=utf8 \
...
if [ ${ALL_REINSTALL} -eq 1 ] || [ ! -d ${MYSQL_DIR} ]; then
    install_mysql
    add_install_record ${MYSQL_ID_NAME} ${MYSQL_TAR_NAME} ${MYSQL_DIR}

else
    echo
    echo "${MYSQL_TAR_NAME} has already been installed."
    echo "Nothing to do."
    echo
fi
