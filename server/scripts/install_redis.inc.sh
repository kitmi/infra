...
function install_redis()
{
    ...
    sed -i "s:__REDIS_USER__:${REDIS_USER}:g"  /etc/init.d/${REDIS_ID_NAME}
    ...
    [ $? -eq 0 ] || exit_with_error "${REDIS_TAR_NAME} cannot be started!"
...
if [ ${ALL_REINSTALL} -eq 1 ] || [ ! -d ${REDIS_DIR} ]; then
    install_redis
    add_install_record ${REDIS_ID_NAME} ${REDIS_TAR_NAME} ${REDIS_DIR}

else
    echo
    echo "${REDIS_TAR_NAME} has already been installed."
    echo "Nothing to do."
    echo
fi
